...
class HtmlTable:
    ...
    def __init__(self):
        self._store = TreeStore(builder=TableBuilder())
        self._table = self._store.table()
        self._thead: TreeStore | None = None
        self._tbody: TreeStore | None = None

    @property
    def store(self) -> TreeStore:
        """Access underlying TreeStore."""
        return self._store

    @property
    def table(self) -> TreeStore:
        """Access table TreeStore."""
        return self._table

    def add_header(self, cells: list[str], **row_attrs) -> TreeStore:
        """Add header row.
    ...
    def add_row(self, cells: list, **row_attrs) -> TreeStore:
        """Add data row.
